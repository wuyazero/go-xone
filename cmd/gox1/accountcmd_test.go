...
func TestAccountListEmpty(t *testing.T) {
	gox1 := runGoX1(t, "account", "list")
	gox1.ExpectExit()
}

func TestAccountList(t *testing.T) {
	datadir := tmpDatadirWithKeystore(t)
	gox1 := runGoX1(t, "account", "list", "--datadir", datadir)
	defer gox1.ExpectExit()
...
func TestAccountNew(t *testing.T) {
	gox1 := runGoX1(t, "account", "new", "--lightkdf")
	defer gox1.ExpectExit()
	gox1.Expect(`
...
`)
	gox1.ExpectRegexp(`Address: \{[0-9a-f]{40}\}\n`)
}
...
func TestWalletImport(t *testing.T) {
	gox1 := runGoX1(t, "wallet", "import", "--lightkdf", "testdata/guswallet.json")
	defer gox1.ExpectExit()
	gox1.Expect(`
!! Unsupported terminal, password will be echoed.
...
`)
	...
	files, err := ioutil.ReadDir(filepath.Join(gox1.Datadir, "keystore"))
	if len(files) != 1 {
...
`)
	...
	for _, m := range wantMessages {
		if !strings.Contains(gox1.StderrText(), m) {
...
// https://github.com/wuyazero/go-xone/issues/1785
...
func TestUnlockFlagAmbiguous(t *testing.T) {
	...
	gox1.SetTemplateFunc("keypath", func(file string) string {
