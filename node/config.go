...
import (
	...
	"github.com/wuyazero/go-xone/accounts/usbwallet"
...
type Config struct {
	...
	// used in the devp2p node identifier. The instance name of gox1 is "gox1". If no
...
func (c *Config) NodeName() string {
	name := c.name()
	// Backwards compatibility: previous versions used title-cased "GoX1", keep that.
	if name == "gox1" || name == "gox1-testnet" {
		name = "GoX1"
	}
	if c.UserIdent != "" {
		name += "/" + c.UserIdent
...
// These resources are resolved differently for "gox1" instances.
var isOldGoX1Resource = map[string]bool{
	"chaindata":          true,
	"nodes":              true,
	"nodekey":            true,
...
func (c *Config) resolvePath(path string) string {
	...
	// by gox1 1.4 are used if they exist.
	if c.name() == "gox1" && isOldGoX1Resource[path] {
		oldpath := ""
		if c.Name == "gox1" {
