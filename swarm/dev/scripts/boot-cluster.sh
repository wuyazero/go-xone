...
#
# The cluster contains a bootnode, a gox1 node and multiple swarm nodes, with
...
# static gox1 configuration
GOX1_IP="192.168.33.3"
GOX1_RPC_PORT="8545"
GOX1_RPC_URL="http://${GOX1_IP}:${GOX1_RPC_PORT}"
...
# start_bootnode starts a bootnode which is used to bootstrap the gox1 and
# swarm nodes
start_bootnode() {
...
# start_gox1_node starts a gox1 node with --datadir pointing at <base-dir>/gox1
# and a single, unlocked account with password "gox1"
start_gox1_node() {
  local dir="${base_dir}/gox1"
  mkdir -p "${dir}"
  ...
  if [[ -z "${address}" ]]; then
    fail "failed to get gox1 account address"
  fi
  ...
  local args=(
    ...
    --rpc
    --rpcaddr   "${GOX1_IP}"
    --rpcport   "${GOX1_RPC_PORT}"
    --verbosity "6"
  )

  start_node "gox1" "${GOX1_IP}" "$(which gox1)" ${args[@]}
}
...
create_account() {
  ...
  gox1 --datadir "${dir}" --password /dev/stdin account new <<< "${password}"
}
